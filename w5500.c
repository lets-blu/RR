...
PRIVATE void w5500Send(W5500 * this) {
	if (!(this->base._txBuffer) || !(this->base._numOfTxData)) {
		return;
	}
...
PRIVATE void w5500Receive(W5500 * this) {
	...
	if (!length || !(this->base._rxBuffer) || this->base._numOfRxData) {
		return;
	}
	
	if (length > this->base._rxBufferSize) {
		length = this->base._rxBufferSize;
	}
	
	if (recv(__SOCK_TCP, this->base._rxBuffer, length) > 0) {
		this->base._numOfRxData = length;
		this->base.onReceive((struct Ethernet *)this);
	}
}
